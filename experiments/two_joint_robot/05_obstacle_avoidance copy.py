...
def run_simulation():
    ...
    # q_goal = p.calculateInverseKinematics(bodyIndex=robot.id, endEffectorLinkIndex=2, targetPosition=goal.base_position)
    # jointspace_target_rmp = TargetPolicy(alpha=0.1, beta=0.01, c=0.1, goal=q_goal,
    #                           name='joint_space_target', taskmap=IdentityTaskmap())
    # core.add_rmp(jointspace_target_rmp)
    ...
    for frame in fkine.frame_names:
        ...
        obstacle_rmp = CollisionAvoidance(d=data_manager[frame]['distance'], vec=data_manager[frame]['normal_vec'],
                                          eta_rep=0.1*np.e, nu_rep=0.3, eta_damp=0.1, nu_damp=0.6, r=3, c=1e5,
    ...
    # simulation
    for step_idx in range(60*100): # n sec at 100Hz
